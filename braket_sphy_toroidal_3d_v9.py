...
try:
    ...
    print("❌ Erro Crítico: O arquivo 'meissner_core.py' não foi encontrado.")
    print("Certifique-se de que ele está no mesmo diretório e é acessível.")
    sys.exit(1)
...
from scipy.interpolate import interp1d, griddata # Importa griddata para o plot 3D
import matplotlib.pyplot as plt
from mpl_toolkits.mplot3d import Axes3D 
import pandas as pd # Importa pandas para manipulação de CSV no plotting
...
def toroidal_tunneling_circuit_3d_braket(barrier_theta, sphy_perturbation_angle):
    """
    Constrói o circuito Toroidal usando a sintaxe do Braket.
    """
    circuit = Circuit()
    
    # 1. State Preparation (Hadamard em todos)
    circuit.h(QUBITS)

    # 2. TOROIDAL LATTICE: The Active SPHY Field (CZ Gates)
    # Conexões CZ: (0, 1), (1, 3), (2, 3), (3, 2), (3, 1), (2, 0), (0, 2), (1, 3), (2, 0), (3, 1) 
    cz_gates = [
    ...
    # 3. Barrier (RZ Gate no Qubit Alvo)
    circuit.rz(TARGET_QUBIT_INDEX, barrier_theta)
    
    # 4. SPHY Noise/Correction (RZ e RX nos Qubits)
    # RZ no alvo
...
def simulate_frame(frame_data):
    ...
    target_qubit_result = int(measurement_results_str[TARGET_QUBIT_INDEX]) # 0 ou 1
    
    result_raw = target_qubit_result # 1 = sucesso de tunelamento
    ideal_state = 1
    ...
    # Cálculo Heurístico da Fase Z Expectation (PauliZ) para o Log 3D
    z_expectations = []
    for i in range(num_qubits):
        if i == TARGET_QUBIT_INDEX:
             z_expval = (2 * target_qubit_result) - 1.0 
        else:
             # Heurística para simular a variação toroidal
             z_expval = (2 * target_qubit_result) - 1.0 
    ...
    # Retorna o valor atualizado de coherence (para o próximo frame) e o log
...
def plot_3d_sphy_field(csv_filename, fig_filename_3d):
    ...
    # O nome das colunas de fase deve ser o mesmo usado na função execute_simulation_multiprocessing
    phase_cols = [f"Qubit_{i+1}_Phase" for i in range(NUM_WIRES)] 
    # Certifique-se de que as colunas existem antes de tentar acessá-las
    ...
    # Coordenadas 2D para a rede 2x2: (0,0), (0,1), (1,0), (1,1)
    X = np.array([0, 0, 1, 1])
    Y = np.array([0, 1, 0, 1])
    Z = mean_phases 

    # Interpolação para suavizar o gráfico 3D
    ...
    # Garante que o Z-limite é definido, mas impede a divisão por zero se Z for todo zero
    if Z.min() != Z.max():
...
def execute_simulation_multiprocessing(num_qubits, total_frames, barrier_theta, noise_prob=1.0, num_processes=4):
    ...
    # O Pool precisa de um loop para atualizar o valor de sphy_coherence.value no manager.
    # Usaremos um loop for normal com Pool.imap_unordered para manter a atualização sequencial da coerência.
    with Pool(processes=num_processes) as pool:
        ...
        for log_entry, new_coherence, error in tqdm(results, total=total_frames, desc="⏳ Simulating Toroidal SPHY"):
            ...
            if log_entry:
                ...
                # O valor de coerência precisa ser atualizado para ser usado no próximo bloco de entrada
